    ...
    var body:some View {
        VStack(alignment:.leading,spacing:10) {
            HStack {
                ...
                Spacer()
                if let quota=summary.weeklyQuota {Text(quota.complete ? "По всей задаче":"Учтённая часть").font(.caption).foregroundStyle(.secondary)}
            }
            if let quota=summary.weeklyQuota {
                Text(quota.points<0.01 ? "< 0,01%":"≈ \(number(quota.points))%")
                    .font(.system(size:28,weight:.semibold,design:.rounded)).monospacedDigit()
                    .foregroundStyle(quota.complete ? Color.mint:Color.orange).accessibilityIdentifier("task-weekly-quota")
                if quota.points<=100 {ProgressView(value:quota.points,total:100).tint(quota.complete ? .mint:.orange)}
                ...
                } else {Text("От одной недельной квоты Codex").font(.caption).foregroundStyle(.secondary)}
                if !quota.complete {
